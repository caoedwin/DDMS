...
class PICS(models.Model):
    ...
    pic = models.ImageField(upload_to='DeviceAPDQATPE/PIC/',verbose_name='图片地址')
...
class DeclarationNofile(models.Model):
    # id = models.AutoField(max_length=10, primary_key=True, verbose_name='id')
    files = models.ImageField(upload_to='DeviceAPDQATPE/DeclarationNo/',verbose_name='文件内容')
...
class DeviceAPDQATPE(models.Model):
    Customer_list = (
        ('', ''),
        ('APDQATPE', 'APDQATPE'),
    )
    ...
    class Meta:
        verbose_name = 'Device_APDQATPE'#不写verbose_name, admin中默认的注册名会加s
        verbose_name_plural = verbose_name
    def __str__(self):
        return '{NID}>>{DevID}'.format(NID=self.NID, DevID=self.DevID)

class DeviceAPDQATPEHis(models.Model):
    Result_choice = (
        # ('Select Customer', 'Select Customer'),
        # ('', ''),
        ('Pass', 'Pass'),
