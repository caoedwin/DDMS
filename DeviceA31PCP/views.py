...
from .models import DeviceA31PCP, DeviceA31PCPHis, PICS, DeviceIntfCtgryList, DeviceDevCtgryList, DeviceDevpropertiesList, DeviceDevVendorList, DeviceDevsizeList
...
@csrf_exempt
def BorrowedDeviceA31PCP(request):
    if not request.session.get('is_login_DMS', None):
        # print(request.session.get('is_login', None))
        return redirect('/login/')
    weizhi = "DeviceA31PCP/BorrowedDevice"
    mock_data = [
    ...
    if DeviceA31PCP.objects.all():
        for i in DeviceA31PCP.objects.values("BrwStatus").distinct().order_by("BrwStatus"):
    ...
    if request.method == "POST":
        if 'first' in str(request.body):
            ...
            if checkAdaPow:
                # print(checkAdaPow)
                # mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow)
                if "IntfCtgry" in checkAdaPow.keys() and "DevCtgry" in checkAdaPow.keys() and "Devproperties" in checkAdaPow.keys() and "DevVendor" in checkAdaPow.keys() and "Devsize" in checkAdaPow.keys():
                    mock_datalist = DeviceA31PCP.objects.filter(
                    ...
                    mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow).filter(DevStatus__in=["Good", "Fixed", 'Long'])
            else:
                mock_datalist = DeviceA31PCP.objects.all().filter(DevStatus__in=["Good", "Fixed", 'Long'])
            # print(mock_datalist)
            ...
            for i in BorrowedID.split(','):
                ...
                try:
                    with transaction.atomic():
                        # print(updatedic)
                        DeviceA31PCP.objects.filter(id=i).update(**updatedic)
                        alert = 0
                except:
                    # print('2')
    ...
    return render(request, 'DeviceA31PCP/BorrowedDeviceA31PCP.html', locals())
...
def R_Borrowed(request):
    if not request.session.get('is_login_DMS', None):
        # print(request.session.get('is_login', None))
        return redirect('/login/')
    weizhi = "DeviceA31PCP/R_Borrowed"
    mock_data = [
        # {"id": "1", "Customer": "C38", "Plant": "KS",
        #  "NID": "1513", "DevID": "UKB0022B", "IntfCtgry": "USB_A",
        #  "DevCtgry": "keyboard", "Devproperties": "USB1.0", "Devsize": "--",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = DeviceA31PCP.objects.filter(Usrname=request.session.get('CNname_DMS'),
                ...
                # else:
                #     mock_datalist = DeviceA31PCP.objects.all()
                # print(mock_datalist)
                for i in mock_datalist:
                    # Photolist = []
                    # for h in i.Photo.all():
            ...
            if request.POST.get('isGetData') == 'SEARCH5':
                ...
                # mock_data
                if IntfCtgry and IntfCtgry != "All" and Devproperties and Devproperties != "All":
                    mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPowfirst).filter(
                    ...
                    mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPowfirst)
                if checkAdaPow:
                    # print(checkAdaPow)
                    # mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow)
                    mock_datalist = mock_datalist.filter(**checkAdaPow)
            ...
            if request.POST.get('isGetData') == 'RENEW':
                ...
                for i in RenewId.split(','):
                    updatedic['Last_BR_per'] = DeviceA31PCP.objects.filter(id=i).first().Usrname
                    updatedic['Last_BR_per_code'] = DeviceA31PCP.objects.filter(id=i).first().BR_per_code
                    updatedic['Last_Predict_return'] = DeviceA31PCP.objects.filter(id=i).first().Plandate
                    updatedic['Last_Borrow_date'] = DeviceA31PCP.objects.filter(id=i).first().Btime
                    updatedic['Last_Return_date'] = datetime.datetime.now().date()
                    updatedic['Last_ProjectCode'] = DeviceA31PCP.objects.filter(id=i).first().ProjectCode
                    updatedic['Last_Phase'] = DeviceA31PCP.objects.filter(id=i).first().Phase
                    try:
                        with transaction.atomic():
                            DeviceA31PCP.objects.filter(id=i).update(**updatedic)
                            alert = 0
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/R_Borrowed.html', locals())
...
def R_Return(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = DeviceA31PCP.objects.filter(Usrname=request.session.get('CNname_DMS'), BR_per_code=request.session.get('account_DMS'), BrwStatus='歸還確認中')
                # mock_data
                # if checkAdaPow:
                #     # print(checkAdaPow)
                #     # mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow)
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/R_Return.html', locals())
...
def R_Keep(request):
    ...
    if request.method == "POST":
            ...
            try:
                ...
                if 'CancelRenew' in str(request.body):
                    ...
                    for i in responseData['Renew']:
                        updatedic['Usrname'] = DeviceA31PCP.objects.filter(id=i).first().Last_BR_per
                        updatedic['BR_per_code'] = DeviceA31PCP.objects.filter(id=i).first().Last_BR_per_code
                        updatedic['ProjectCode'] = DeviceA31PCP.objects.filter(id=i).first().Last_ProjectCode
                        updatedic['Phase'] = DeviceA31PCP.objects.filter(id=i).first().Last_Phase
                        updatedic['Plandate'] = DeviceA31PCP.objects.filter(id=i).first().Last_Predict_return
                        updatedic['Btime'] = DeviceA31PCP.objects.filter(id=i).first().Last_Borrow_date
                        updatedic['Rtime'] = DeviceA31PCP.objects.filter(id=i).first().Last_Return_date
                        try:
                            with transaction.atomic():
                                # print(updatedic)
                                DeviceA31PCP.objects.filter(id=i).update(**updatedic)
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/R_Keep.html', locals())
...
def R_Destine(request):
    ...
    if request.method == "POST":
        if request.POST:
            ...
            if 'first' in str(request.body):
                # mock_data
                mock_datalist = DeviceA31PCP.objects.filter(Usrname=request.session.get('CNname_DMS'), BR_per_code=request.session.get('account_DMS'), BrwStatus='預定確認中')
                # if checkAdaPow:
                #     # print(checkAdaPow)
                #     # mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow)
                #     if "IntfCtgry" in checkAdaPow.keys() and "DevCtgry" in checkAdaPow.keys() and "Devproperties" in checkAdaPow.keys() and "DevVendor" in checkAdaPow.keys() and "Devsize" in checkAdaPow.keys():
            ...
            try:
                ...
                if 'CancelBook' in str(request.body):
                    ...
                    for i in responseData['BookId']:
                        ...
                        updatedic['ProjectCode'] = DeviceA31PCP.objects.filter(id=i).first().Last_ProjectCode
                        updatedic['Phase'] = DeviceA31PCP.objects.filter(id=i).first().Last_Phase
                        # updatedic['Predict_return'] = DeviceA31PCP.objects.filter(id=i).first().Last_Predict_return
                        updatedic['Btime'] = DeviceA31PCP.objects.filter(id=i).first().Last_Borrow_date
                        updatedic['Rtime'] = DeviceA31PCP.objects.filter(id=i).first().Last_Return_date
                        # print(updatedic)
                        try:
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/R_Destine.html', locals())
...
def M_Borrow(request):
    if not request.session.get('is_login_DMS', None):
        # print(request.session.get('is_login', None))
        return redirect('/login/')
    weizhi = "DeviceA31PCP/M_Borrow"
    selectItem = [
        # {"value": "姚麗麗", "number": "12345678"}, {"value": "張亞萍", "number": "11111111"}, {"value": " 錢剛", "number": "33445566"},
    ]
    for i in DeviceA31PCP.objects.filter(BrwStatus='預定確認中').values('Usrname', 'BR_per_code').distinct().order_by('BR_per_code'):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = DeviceA31PCP.objects.filter(BrwStatus='預定確認中')
                for i in mock_datalist:
                    # Photolist = []
                    # for h in i.Photo.all():
                    #     Photolist.append(
            ...
            try:
                ...
                if 'ChangeDelivery' in str(request.body):
                    ...
                    for i in responseData['params']:
                        ...
                        # updatedic['Last_Return_date'] = DeviceA31PCP.objects.filter(id=i).first().Return_date
                        try:
                            with transaction.atomic():
                                # print(updatedic)
                                DeviceA31PCP.objects.filter(id=i).update(**updatedic)
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/M_Borrow.html', locals())
...
def M_Return(request):
    ...
    for i in DeviceA31PCP.objects.filter(BrwStatus='歸還確認中').values('Usrname', 'BR_per_code').distinct().order_by(
            'BR_per_code'):
        selectItem.append({"value": i['Usrname'], 'number': i['BR_per_code']})
    mock_data = [
    ...
    if request.method == "POST":
        if request.POST:
            ...
            try:
                ...
                if 'ChangeStorages' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # updatedic['Last_BR_per'] = DeviceA31PCP.objects.filter(id=i).first().BR_per
                        # updatedic['Last_Predict_return'] = DeviceA31PCP.objects.filter(id=i).first().Predict_return
                        # updatedic['Last_Borrow_date'] = datetime.datetime.now().date(),
                        updatedic['Last_Return_date'] = datetime.datetime.now().date()

                        if DeviceA31PCP.objects.filter(id=i).first().uscyc:
                            uscyc = int(DeviceA31PCP.objects.filter(id=i).first().uscyc)
                        else:
                            uscyc = 0
                        if DeviceA31PCP.objects.filter(id=i).first().UsrTimes:
                            UsrTimes = int(DeviceA31PCP.objects.filter(id=i).first().UsrTimes)
                        else:
                            UsrTimes = 0
                        updatedic['uscyc'] = str(uscyc + int(DeviceA31PCP.objects.filter(id=i).first().Last_uscyc))
                        updatedic['UsrTimes'] = str(UsrTimes + 1)

                        Devicebyid = DeviceA31PCP.objects.filter(id=i).first()
                        updatedic_His = {
                            "NID": Devicebyid.NID, "DevID": Devicebyid.DevID,
                            "DevModel": Devicebyid.DevModel, "DevName": Devicebyid.DevName,
                            "uscyc": DeviceA31PCP.objects.filter(id=i).first().Last_uscyc,
                        ...
                        try:
                            with transaction.atomic():
                                if DeviceA31PCPHis.objects.filter(**updatedic_His_check):
                                    pass#防止历史记录存成功了，Device更新失败，需要重复操作。
                                else:
                                    DeviceA31PCPHis.objects.create(**updatedic_His)
                                DeviceA31PCP.objects.filter(id=i).update(**updatedic)
                                alert = 0
                        except:
                            alert = '此数据%s正被其他使用者编辑中...' % i
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/M_Return.html', locals())
...
def M_Keep(request):
    if not request.session.get('is_login_DMS', None):
        # print(request.session.get('is_login', None))
        return redirect('/login/')
    weizhi = "DeviceA31PCP/M_Keep"

    selectItem = [
        # {"value": "姚麗麗", "number": "12345678"}, {"value": "張亞萍", "number": "11111111"},
        # {"value": " 錢剛", "number": "33445566"},
    ]
    for i in DeviceA31PCP.objects.filter(BrwStatus='續借確認中').values('Usrname', 'BR_per_code').distinct().order_by('BR_per_code'):
        selectItem.append({"value": i['Usrname'],'number': i['BR_per_code']})
    mock_data = [
        # {"id": "2", "Customer": "C38", "Plant": "KS",
        #  "NID": "1514", "DevID": "UKB0022B", "IntfCtgry": "USB_A",
    ...
    if request.method == "POST":
            ...
            try:
                ...
                if 'EnsureRenew' in str(request.body):
                    ...
                    for i in responseData['params']:
                        ...
                        "NID": Devicebyid.NID, "DevID": Devicebyid.DevID,
                        "DevModel": Devicebyid.DevModel, "DevName": Devicebyid.DevName,
                        # "uscyc": DeviceA31PCP.objects.filter(id=i).first().uscyc,
                        "Btime": Devicebyid.Last_Borrow_date, "Plandate": Devicebyid.Last_Predict_return,
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/M_Keep.html', locals())
...
def M_Category(request):
    ...
    if request.method == "POST":
        ...
        # print(data)
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/M_Category.html', locals())
...
def M_edit(request):
    ...
    if DeviceA31PCP.objects.all():
        for i in DeviceA31PCP.objects.values("DevStatus").distinct().order_by("DevStatus"):
    ...
    errMsg = ''
    errMsgNumber = ''#新增弹框
    # print(DeviceA31PCP._meta.fields)
    # # print([f.name for f DeviceA31PCP._meta.fields])
    # iii=0
    # for i in DeviceA31PCP._meta.fields:
    #     print(i.name, type(i), i.get_internal_type())
        # iii+=1
    # print(iii)
    # print(DeviceA31PCP._meta.get_fields())
    # print(request.method)
    if request.method == "POST":
        # print(request.POST)
        # print(request.body)
        if request.POST:
            ...
            if request.POST.get('isGetData') == 'SEARCH':
                ...
                #     # mock_datalist = DeviceA31PCP.objects.filter(**checkAdaPow)
                #     if "IntfCtgry" in checkAdaPow.keys() and "DevCtgry" in checkAdaPow.keys() and "Devproperties" in checkAdaPow.keys() and "DevVendor" in checkAdaPow.keys() and "Devsize" in checkAdaPow.keys():
                #         mock_datalist = DeviceA31PCP.objects.filter(Q(IntfCtgry__icontains=checkAdaPow['IntfCtgry'])&Q(DevCtgry=checkAdaPow['DevCtgry'])
            ...
            if request.POST.get('action') == "submit":
                ...
                # print(createdic)
                if DeviceA31PCP.objects.filter(NID=NID):
                    errMsgNumber = """設備序號已存在"""
                else:
                    # print(createdic)
                    DeviceA31PCP.objects.create(**createdic)
                    # print(Photo)
                    # if Photo:
                    ...
                    #         empt.pic = f
                    #         empt.save()
                    #         DeviceA31PCP.objects.filter(Number=Number).first().Photo.add(empt)
            ...
            if request.POST.get('action') == "submit1":
                ...
                # if DeviceA31PCP.objects.filter(Number=Number):
                #     errMsgNumber = """编号已存在"""
                # else:
                # print(updatedic)
                try:
                    with transaction.atomic():
                        DeviceA31PCP.objects.filter(id=id).update(**updatedic)
                        alert = 0
                except:
                    alert = '此数据%s正被其他使用者编辑中...' % id
                # print(Photo)
                # if Photo:
                #     for m in DeviceA31PCP.objects.filter(id=id).first().Photo.all():  # 每次接受图片前清除原来的图片，而不是叠加
                #         # print(m.id)
                #         PICS.objects.filter(
                #             id=m.id).delete()
                #     for f in Photo:  # 数据上允许传多张照片，实际生产环境中头像只会传一张
                ...
                #         empt.pic = f
                #         empt.save()
                #         DeviceA31PCP.objects.filter(id=id).first().Photo.add(empt)
            ...
            if request.POST.get('isGetData') == "alertData":
                ...
                NID = request.POST.get('NID')
                # print(NID)
                if DeviceA31PCPHis.objects.filter(NID=NID):
                    for i in DeviceA31PCPHis.objects.filter(NID=NID):
                        if i.Btime:
                            Btime_str = str(i.Btime)
                        else:
                            Btime_str = ''
            ...
            try:
                ...
                if 'MUTICANCEL' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # print(i)
                        DeviceA31PCP.objects.get(id=i).delete()
                ...
                if 'ExcelData' in str(request.body):
                    ...
                    if startupload:
                        for i in uploadxlsxlist:
                            ...
                            if DeviceA31PCP.objects.filter(**Check_dic):#已经存在覆盖
                                # pass
                                DeviceA31PCP.objects.filter(
                                    **Check_dic).update(**i)
                            else:#新增
                                DeviceA31PCP.objects.create(**i)
                        errMsg = '上傳成功'
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceA31PCP/M_edit.html', locals())
