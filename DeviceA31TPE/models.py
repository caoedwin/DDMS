...
class PICS(models.Model):
    ...
    pic = models.ImageField(upload_to='DeviceA31TPE/PIC/',verbose_name='图片地址')
...
class DeclarationNofile(models.Model):
    # id = models.AutoField(max_length=10, primary_key=True, verbose_name='id')
    files = models.ImageField(upload_to='DeviceA31TPE/DeclarationNo/',verbose_name='文件内容')
...
class DeviceA31TPE(models.Model):
    Customer_list = (
        ('', ''),
        ('C38(NB)', 'C38(NB)'),
        ('C38(AIO)', 'C38(AIO)'),
...
class DeviceA31TPEHis(models.Model):
    Result_choice = (
        # ('Select Customer', 'Select Customer'),
        # ('', ''),
        ('Pass', 'Pass'),
